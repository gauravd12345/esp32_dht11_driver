...
/* 
    DHT11 COMMUNICATION PROCESS(FROM DATASHEET):
        1. Initiating START condition: 
                MCU has to pull down (3.3V) DATA Line for at least 18ms and then pull up for 20-40µs

        2. DHT11 detects START: 
                DHT11 pulls down DATA for 80µs acknowledging the START condition, then it pulls DATA high for 80µs, 
                and finally it pulls DATA low for 50µs as it prepares for data transmission.

        3. Sending data: 
                Data is sent by the DHT11 sensor in a pair. First is a low voltage level that lasts 50µs 
